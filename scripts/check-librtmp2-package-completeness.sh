...
check_apt() {
    local missing=0
    local target root codename arch package_version
    local -a targets=(
    ...
    for target in "${targets[@]}"; do
        ...
        check_file "$root/dists/$codename/main/binary-$arch/Packages" || missing=1
    ...
    for target in "debian:bookworm" "debian:trixie" "ubuntu:jammy" "ubuntu:noble" "ubuntu:resolute"; do
        ...
        check_file "$root/dists/$codename/Release.gpg" || missing=1
    done

    if (( missing != 0 )); then
        echo "APT repository for librtmp2 $VERSION is incomplete."
        return 1
    fi

    echo "APT repository for librtmp2 $VERSION is complete."
}
...
check_alpine() {
    ...
    for branch in "${branches[@]}"; do
        for arch in "${arches[@]}"; do
            ...
            check_glob "$base/librtmp2-${VERSION}-r*.apk" || missing=1
            check_glob "$base/librtmp2-dev-${VERSION}-r*.apk" || missing=1
            check_glob "$base/librtmp2-static-${VERSION}-r*.apk" || missing=1
            check_file "$base/APKINDEX.tar.gz" || missing=1
        done
    done

    if (( missing != 0 )); then
        echo "Alpine repository for librtmp2 $VERSION is incomplete."
        return 1
    fi

    echo "Alpine repository for librtmp2 $VERSION is complete."
}
